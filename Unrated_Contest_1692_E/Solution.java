...
public class Main {
    ...
    public static void solve(Scanner sc, ArrayList<String> ans) {
        ...
        for (int i = s; i <= sum; i++) {
            int rightBoundary = lastOcc[i];
            int leftBoundary = firstOcc[i - s]; 

            int removedFromLeft = leftBoundary;
            int removedFromRight = n - rightBoundary;

            result = Math.min(result, removedFromLeft + removedFromRight);
        }
