...
public class Main {
    ...
    public static void solve(Scanner sc, ArrayList<String> ans) {
        ...
        if(n==1)
        {
            ans.add(Long.toString(nums[0]));
            return;
        }
        long oddGCD=nums[1],evenGCD=nums[0];
        for (int i = 0; i < n; i++) {
            if(i%2==0) evenGCD=gcd(evenGCD, nums[i]);
            else oddGCD=gcd(oddGCD, nums[i]);
        }
        boolean odd=true,even=true  ;
        for (int i = 0; i < n; i++) {
            if(i%2==0)
            {
                if(nums[i]%oddGCD==0) odd=false;
            }
            else{
                if(nums[i]%evenGCD==0) even =false;
            }
        }
        if(!odd && !even)
            ans.add(Integer.toString(0));
        else if(odd)
            ans.add(Long.toString(oddGCD));
        else
            ans.add(Long.toString(evenGCD));
