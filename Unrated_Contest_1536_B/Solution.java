...
public class Main {
    ...
    public static void solve(Scanner sc, ArrayList<String> ans) {
        int n = sc.nextInt();
        String s = sc.next(); // Use next() to avoid nextLine() spacing issues

        // A queue to generate strings in lexicographical and length order
        Queue<String> queue = new LinkedList<>();

        // Seed the queue with length-1 strings
        for (char c = 'a'; c <= 'z'; c++) {
            queue.add(String.valueOf(c));
        }

        // Breadth-First Search for the MEX
        while (!queue.isEmpty()) {
            String curr = queue.poll();

            // If the original string does NOT contain this substring, we found the MEX!
            if (!s.contains(curr)) {
                ans.add(curr);
                return;
            }

            // If it does exist, append a-z to the current string and queue it for later
            for (char c = 'a'; c <= 'z'; c++) {
                queue.add(curr + c);
            }
        }
